...
class TestHero(TestCase):
    ...
    def test_battle_raises_when_attacker_attack_enemy_with_same_username(self):
    ...
    def test_battle_returns_win_when_enemy_dies(self):
        enemy_level, enemy_heath, enemy_damage = 5, 100, 10
        enemy = Hero("Enemy", enemy_level, enemy_heath, enemy_damage)

        result = self.attacker.battle(enemy)
        expected_enemy_health = enemy_heath - (self.HERO_LEVEL * self.HERO_DAMAGE)
    ...
    def test_battle_returns_defeat_when_attacker_dies(self):
        attacker_level, attacker_heath, attacker_damage = 5, 100, 10
        attacker = Hero("Attacker", attacker_level, attacker_heath, attacker_damage)

        enemy = Hero("Enemy", self.HERO_LEVEL, self.HERO_HEALTH, self.HERO_DAMAGE)

        result = attacker.battle(enemy)

        expected_attacker_health = attacker_heath - (self.HERO_LEVEL * self.HERO_DAMAGE)
    ...
    def test_str_returns_proper_message(self):
