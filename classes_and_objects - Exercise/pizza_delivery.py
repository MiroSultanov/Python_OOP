...
# o	If we have the ingredient, but we try to remove more than we have available, we should return the following message "Please check again the desired quantity 
# of {ingredient}!"
...
# -	make_order()
# o	Set the attribute ordered to True and return the following message "You've ordered pizza {pizza_name} prepared with {ingredient: quantity} and the price 
# will be {price}lv.". The ingredients should be separated by a comma and a space ", "
# o	Keep in mind that once the pizza is ordered, no further changes are allowed. We should return the following message if the customer tries to change
# it: "Pizza {name} already prepared, and we can't make any changes!"
...
class PizzaDelivery:
    ...
    def remove_ingredient(self, ingredient: str, quantity: int, price_per_quantity: float):
        if self.ordered:
            return f"Pizza {self.name} already prepared, and we can't make any changes!"
        current_quantity = self.ingredients.get(ingredient)
    ...
    def make_order(self):
        self.ordered = True
        new_ingredients = ', '.join([f"{key}: {value}" for key, value in self.ingredients.items()])
        return f"You've ordered pizza {self.name} prepared with {new_ingredients} and the price will be {self.price}lv."
